...
router.get('/twitter-login', async (ctx) => {
	...
	const twitterAuthUrl = await initializeAuthorization(`${serverUrl}/twitter-authorization-callback`, '/');
...
loadDataAndScheduleTasks();
initTwitterKeys(process.env.TWITTER_API_KEY || '', process.env.TWITTER_API_SECRET || '');
